...
SELECT
    ...
    SUM(iv.quantidade * iv.valor_unitario * (1 - iv.desconto / 100.0)) AS faturamento_liquido
FROM itens_venda iv
INNER JOIN produtos p
    ON iv.id_produto = p.id_produto
...
SELECT
    vd.nome_vendedor,
    vv.quantidade_vendas,
    vv.quantidade_clientes_atendidos,
    it.quantidade_produtos_vendidos,
    it.faturamento_dos_itens,
    vv.ticket_medio_das_vendas
FROM vendedores vd
INNER JOIN (
    SELECT
        v.id_vendedor,
    ...
    ON vd.id_vendedor = vv.id_vendedor
INNER JOIN (
    SELECT
        v.id_vendedor,
        SUM(iv.quantidade) AS quantidade_produtos_vendidos,
        SUM(iv.quantidade * iv.valor_unitario * (1 - iv.desconto / 100.0)) AS faturamento_dos_itens
    FROM vendas v
    INNER JOIN itens_venda iv
        ON v.id_venda = iv.id_venda
    WHERE v.status = 'Concluída'
    ...
    ON vd.id_vendedor = it.id_vendedor
ORDER BY it.faturamento_dos_itens DESC;